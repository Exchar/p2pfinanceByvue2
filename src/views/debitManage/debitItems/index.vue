<template>
  <div class="viewMain">
    ...
    <el-row :gutter="20" class="searchLine">
      ...
      ></el-col>
      <el-col :span="4">
        <el-select v-model="value" placeholder="全部状态" @change="searchLoan">
        ...
        <template slot-scope="scope">
          ...
          <el-dialog title="借款记录详情" :visible.sync="dialogFormVisible">
            <el-form :model="form">
              ...
              <el-form-item label="借款管理费：" :label-width="formLabelWidth">
                <span>{{ form.managerfee?'￥'+parseFloat(form.managerfee).toFixed(2):'' }}</span>
              </el-form-item>
          ...
          <el-dialog title="投资记录详情" :visible.sync="dialogForm1Visible">
            ...
            <el-form :model="form1" v-show="form1.id?true:false">
              ...
              <el-form-item label="投资金额" :label-width="formLabelWidth">
                <span>{{ form1.money?'￥'+parseFloat(form1.money).toFixed(2):'' }}</span>
              </el-form-item>
              <el-form-item label="利息" :label-width="formLabelWidth">
                <span>{{ form1.interest?'￥'+form1.interest* 100 + "%":'' }}</span>
              </el-form-item>
              <el-form-item label="收益方式" :label-width="formLabelWidth">
                <span>{{ form1.income }}</span>
              </el-form-item>
              <el-form-item label="借款管理费" :label-width="formLabelWidth">
                <span>{{ form1.managerfee?'￥'+parseFloat(form1.managerfee).toFixed(2):'' }}</span>
              </el-form-item>
              <el-form-item label="收益方式" :label-width="formLabelWidth">
                <span>{{ form1.income }}</span>
              </el-form-item>
              <el-form-item label="已收金额" :label-width="formLabelWidth">
                <span>{{ form1.amount?'￥'+parseFloat(form1.amount).toFixed(2):'' }}</span>
              </el-form-item>
              <el-form-item label="待收总额" :label-width="formLabelWidth">
                <span>{{ form1.duein?'￥'+parseFloat(form1.duein).toFixed(2):'' }}</span>
              </el-form-item>
              ...
              <el-form-item label="投资时间" :label-width="formLabelWidth">
                <span>{{ form1.investime?(form1.investime|formatDate):'' }}</span>
              </el-form-item>
            ...
            <div slot="footer" class="dialog-footer">
              <el-button type="primary" @click="closeInvest"
                >关闭</el-button
              >
            </div>
          </el-dialog>
          ...
          <el-dialog title="还款记录详情" :visible.sync="dialogForm2Visible">
            ...
            <el-form :model="form2" v-show="form2.debitNum?true:false">
              ...
              <el-form-item label="借款金额" :label-width="formLabelWidth">
                <span>{{ form2.money?'￥'+parseFloat(form2.money).toFixed(2):'' }}</span>
              </el-form-item>
              <el-form-item label="期限" :label-width="formLabelWidth">
                <span>{{ form2.backdate?new Date(form2.backdate).toLocaleDateString():'' }}</span>
              </el-form-item>
              <el-form-item label="还款方式" :label-width="formLabelWidth">
              ...
              <el-form-item label="应还款时间" :label-width="formLabelWidth">
                <span>{{form2.returntime?new Date(form2.returntime).toLocaleDateString():'' }}</span>
              </el-form-item>
              <el-form-item label="实际还款时间" :label-width="formLabelWidth">
                <span>{{ form2.realtime?new Date(form2.realtime).toLocaleDateString():'' }}</span>
              </el-form-item>
              <el-form-item label="预还金额" :label-width="formLabelWidth">
                <span>{{ form2.returnmoney?'￥'+parseFloat(form2.returnmoney).toFixed(2):'' }}</span>
              </el-form-item>
              <el-form-item label="实还金额" :label-width="formLabelWidth">
               <span>{{ form2.realmoney?'￥'+parseFloat(form2.realmoney ).toFixed(2):''}}</span>
              </el-form-item>
              <el-form-item label="本金" :label-width="formLabelWidth">
                <span>{{ form2.capital?'￥'+parseFloat(form2.capital).toFixed(2):'' }}</span>
              </el-form-item>
              <el-form-item label="利息" :label-width="formLabelWidth">
                <span>{{ form2.interest?'￥'+parseFloat(form2.interest).toFixed(2):'' }}</span>
              </el-form-item>
              <el-form-item label="逾期罚息" :label-width="formLabelWidth">
                <span>{{form2.punish?'￥'+parseFloat(form2.punish).toFixed(2):'' }}</span>
              </el-form-item>
            ...
            <div slot="footer" class="dialog-footer">
              <el-button type="primary" @click="closepayback"
                >关闭</el-button
              >
            </div>
          </el-dialog>
